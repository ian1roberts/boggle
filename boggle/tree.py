...
class Tree(object):
    ...
    def __init__(self, ori, wlen, nrow, ncol):
        ...
        self.tree = dict(zip([x for x in range(0,wlen)], [{} for x in range(0,wlen)]))
        # Route through grid
        self.compute_tree()
        self.build_paths()
        self.prune_nodes()
    ...
    def _get_valid_moves(self, loc):
        ' computes valid compass points from an x,y location'
        R, C = loc
        vmoves = []
        ...
        for tier in range(0, self.wlen-1):
            for node in self.tree[tier]:
                ...
                for move in parent.moves:
                    ...
                    self.tree[tier+1][(parent.loc, move, str(uuid.uuid4())[:6])] = Leaves(tier+1, parent, loc, next_moves)
    ...
    def build_paths(self):
        ...
        for z,key in enumerate(pk):
            if z % 10000 == 0:
                print("Done {} of {} ({:.2f}%)".format(1+z, len(pk), float(1+z)/len(pk)*100))
